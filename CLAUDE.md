...
- `sync.py` -- Orchestrator. Three type-specific column mappers and three type-specific update-body formatters extract fields from `Finding.raw` dict. Routes findings to the correct board, creates the item, posts the Updates-feed body. With `--set-triage-reviewing`, also triages the finding in Semgrep (state="reviewing" + note with monday item URL). With `--dry-run`, fetches findings and prints IDs by type without creating monday items, updating state, or triaging. With `--mark-fixed` (SAST + SCA only), runs `mark_fixed()` **after** the normal sync as a reconciliation pass. `mark_fixed()` bulk-fetches Semgrep's active-projects list once (server-side excludes archived repos) and calls the **v2 Issues API** (`POST /api/agent/deployments/{id}/issues`) once per board type with filter `{aggregateIssueStates: [AGGREGATE_ISSUE_STATE_FIXED], onPrimaryBranch: true}` plus optional `timeFilter: TIME_FILTER_FIXED_AT` + `since` (from `--fixed-since-days N`). Any state.json entry whose repo is absent from the active-projects list is marked "Not scanned by Semgrep" and migrated to `not_scanned_state.json`. Any state.json entry whose `finding_ids` intersect the fetched fixed-on-primary set is marked "Fixed" and migrated to `fixed_state.json`. Backfill: pre-v5 entries have `repo=""`; `mark_fixed()` fills them once via monday's "Repo" column (`get_items_by_ids`, batched to the 100-id cap with an explicit `limit: 100` since monday defaults to 25) at the start of the pass. Loads filters and passes them as `extra_params` to each fetch call. When `malicious: [true]` is set in the SCA filter block, runs a second SCA fetch with only `is_malicious=true` (no other filters) and merges results with the primary fetch (deduped by finding ID). For Secrets, always runs a second fetch with `aggregateIssueStates: [AGGREGATE_ISSUE_STATE_FIXED]` (inheriting all user scoping filters except `tab`); fixed findings whose `note` contains `monday.com` are skipped — they were previously reviewed and already have a monday item. This captures secrets fixed before the sync ever ran. The fixed fetch deliberately excludes `max_findings` from `fetch_kwargs` so `--limit` does not cap it — fixed secrets are paginated to completion regardless of limit. Because the Semgrep triage API silently ignores note writes on FIXED findings, a separate `fixed_state.json` file tracks synced fixed items. It is loaded at the start of `run()`, checked during fixed-findings filtering (alongside the note check), updated on successful item creation, and saved at the end. Format v3: `{"version": 3, "SECRETS": [fid, ...], "SAST": {item_id: {"repo": "...", "finding_ids": [...]}}, "SCA": {...}}`. v1 (secrets-only list) and v2 (flat SAST/SCA lists) are auto-migrated on load. `not_scanned_state.json` uses the same per-item shape (v1): `{"version": 1, "SAST": {item_id: {"repo", "finding_ids"}}, "SCA": {...}}`. Both files' paths are derived as siblings of `state_path`.
...
- State v5 format: `{"version": 5, "monday_items_created": {"SAST": {"item_id": {"repo": "org/name", "finding_ids": ["fid1", "fid2"]}}, "SCA": {...}, "Secrets": {...}}, "daily": {...}}`. Top-level keys are board types; each maps monday.com item IDs to per-item entries carrying the repo name and the list of Semgrep finding IDs. `repo` is populated at item-creation time by `run()` (from `finding.repo`) and is backfilled lazily by `mark_fixed()` for pre-v5 entries via monday's "Repo" column. v1–v4 are auto-migrated on load (v4 entries land with `repo=""` and get backfilled on the next `mark_fixed` run). `synced_finding_ids(state, board_type)` returns a per-type set for O(1) dedup lookups. Filters never modify state — they gate new fetches only.
...
- SAST fetch runs twice: `issue_type=sast` then `issue_type=ai_sast`. Results are merged into the same SAST board, deduplicated by finding ID. Both calls use the same SAST filter block.
